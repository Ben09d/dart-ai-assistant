...
const DART_METHODS: Record<string, Array<{ label: string; detail: string; snippet: string }>> = {
    'String': [
        { label: 'contains()', detail: 'bool contains(Pattern other)', snippet: 'contains(\'$1\')' },
        { label: 'split()', detail: 'List<String> split(Pattern pattern)', snippet: 'split(\'$1\')' },
        { label: 'trim()', detail: 'String trim()', snippet: 'trim()' },
        { label: 'toLowerCase()', detail: 'String toLowerCase()', snippet: 'toLowerCase()' },
        { label: 'toUpperCase()', detail: 'String toUpperCase()', snippet: 'toUpperCase()' },
        { label: 'replaceAll()', detail: 'String replaceAll(Pattern from, String replace)', snippet: 'replaceAll(\'$1\', \'$2\')' },
        { label: 'startsWith()', detail: 'bool startsWith(Pattern pattern)', snippet: 'startsWith(\'$1\')' },
        { label: 'endsWith()', detail: 'bool endsWith(String other)', snippet: 'endsWith(\'$1\')' },
        { label: 'isEmpty', detail: 'bool get isEmpty', snippet: 'isEmpty' },
        { label: 'isNotEmpty', detail: 'bool get isNotEmpty', snippet: 'isNotEmpty' },
        { label: 'length', detail: 'int get length', snippet: 'length' },
    ],
    'List': [
        { label: 'add()', detail: 'void add(E value)', snippet: 'add($1)' },
        { label: 'addAll()', detail: 'void addAll(Iterable<E> iterable)', snippet: 'addAll($1)' },
        { label: 'remove()', detail: 'bool remove(Object? value)', snippet: 'remove($1)' },
        { label: 'removeAt()', detail: 'E removeAt(int index)', snippet: 'removeAt($1)' },
        { label: 'where()', detail: 'Iterable<E> where(bool Function(E) test)', snippet: 'where(($1) => $2)' },
        { label: 'map()', detail: 'Iterable<T> map<T>(T Function(E) f)', snippet: 'map(($1) => $2).toList()' },
        { label: 'forEach()', detail: 'void forEach(void Function(E) action)', snippet: 'forEach(($1) {\n  $2\n})' },
        { label: 'sort()', detail: 'void sort([int Function(E, E)? compare])', snippet: 'sort(($1, $2) => $1.compareTo($2))' },
        { label: 'contains()', detail: 'bool contains(Object? element)', snippet: 'contains($1)' },
        { label: 'firstWhere()', detail: 'E firstWhere(bool Function(E) test)', snippet: 'firstWhere(($1) => $2)' },
        { label: 'isEmpty', detail: 'bool get isEmpty', snippet: 'isEmpty' },
        { label: 'length', detail: 'int get length', snippet: 'length' },
        { label: 'first', detail: 'E get first', snippet: 'first' },
        { label: 'last', detail: 'E get last', snippet: 'last' },
    ],
    'Map': [
        { label: 'containsKey()', detail: 'bool containsKey(Object? key)', snippet: 'containsKey($1)' },
        { label: 'containsValue()', detail: 'bool containsValue(Object? value)', snippet: 'containsValue($1)' },
        { label: 'putIfAbsent()', detail: 'V putIfAbsent(K key, V Function() ifAbsent)', snippet: 'putIfAbsent($1, () => $2)' },
        { label: 'remove()', detail: 'V? remove(Object? key)', snippet: 'remove($1)' },
        { label: 'keys', detail: 'Iterable<K> get keys', snippet: 'keys' },
        { label: 'values', detail: 'Iterable<V> get values', snippet: 'values' },
        { label: 'entries', detail: 'Iterable<MapEntry<K,V>> get entries', snippet: 'entries' },
        { label: 'forEach()', detail: 'void forEach(void Function(K, V) action)', snippet: 'forEach(($1, $2) {\n  $3\n})' },
    ],
};
    ...
    private _widgetCompletions(prefix: string, ctx: ContextProfile): CompletionCandidate[] {
        ...
        return FLUTTER_WIDGETS
            ...
            .map(w => ({
                ...
                detail: w.deprecated ? 'Flutter Widget (deprecated)' : 'Flutter Widget',
                documentation: w.doc,
                sortText: `2${w.name}`,
    ...
    $1
        ...
        return hooks
            ...
            .map(h => ({
                ...
                detail: 'StatefulWidget lifecycle',
                documentation: h.doc,
                sortText: `6${h.label}`,
                score: 0.84,
                isSnippet: true,
